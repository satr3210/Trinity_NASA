...
```{r import}
# Import data
df <- import_data(c("2U1D_unlocked","2U1D_locked","1U1D","MFP"))
```
...
fitted <- fit_nlme(
     ...
     start = c(
          asym=c("2U1D_unlocked"=2.04443686, "1U1D" = 1.707104133, "MFP"=1.6822018, "2U1D_locked"=1.6150422),
          scal=c("2U1D_unlocked"=0.31462406,"1U1D" =0.231073265, "MFP"=0.48888491, "2U1D_locked"=0.23575112),
          xmid = 0.157604291
     )
...
ggsave("alg_nlme_curves.jpg")
# Print the model summary
change_intercept(fitted$model$coefficients$fixed)
VarCorr(fitted$model)
```
This model uses a one fixed effect per group for the asymptote and the scale parameters.  It uses a global parameter for the horizontal shift, xmid.   This simplification is required because the xmid parameter does not vary much between groups, and including it causes a numerically singular model as a result.  The individual subjects form the random effects for the asymptote parameter.  We could not fit a model with random effects for the scale parameter due to overparameterization.  

The (Intercept) specifications correspond to the 2 up 1 down unlocked group.  We see that the 2 up 1 down unlocked group attains a substantially higher asymptote than all other groups; all three remaining groups are similar.  Also interestingly, the median fixed progression group, the only non-adaptive algorithm, sees a substantially longer timescale for improvement; the other three adaptive groups are similar.  We can easily see this effect on the plot of the curves themselves.  Also importantly, the asymptote parameters varies significantly per subject, showing a standard deviation more than twice the size of the difference in the fixed effects.  The median fixed progression condition shows the smallest variance about the curve; we expect this behavior because all subjects are subject to the same difficulty control constant at each time while other groups will vary.

These results suggest that the 2 up 1 down unlocked algorithm results in the highest integrated skill achievement.  The median fixed progression results in slower learning.  This effect likely occurs because it, on average, progresses too quickly for half the subjects and too slowly for the other half. Progressing too slowly prevents skill advancement per the difficulty control; progressing too quickly results in poor performance for a longer period of time.  Both effects reduce the rate of progress.  The high variance between subjects suggests that an accurate estimate of the effect from the training condition will require many subjects.
...
ggplot(data=residual_data, aes(sample=res)) +
     ...
     labs(title="QQ plot of NLME (integrated) residuals",
          subtitle="Indicates left skew and skinniness",
          x="Quantile of residuals",
          y="Quantitle of normal"
     )

ggsave("alg_nlme_res_qq.jpg")
...
adj_coeffs <- change_intercept(fitted$model$coefficients$fixed)

set.seed(8293003) # Repeatability
# Use data from training sessions with 1000 iterations on integrated skill.
...
param_dists <- data.frame(perm_res$samples) %>%
    ...
    mutate(
        true = map_vec( 
            param,
            function(el) {adj_coeffs[[el]]}
            )
    )
```
```{r perm_res, fig.height=11, units="in"}
ggplot(data= param_dists, mapping=aes(x=abs(value) )) +
    ...
    labs(title="Histogram of NLME parameter distribution",
        ...
        x="Parameter distance to 2U1D unlocked group",
        y="Count"
    )
ggsave("alg_param_hist.jpg")
print("Permutation test p-values.  Different from 2U1D unlocked?")
perm_res$pvals
```
...
We can see that the scal parameter for median fixed progression group is significantly different from that of the adaptive condition.  Namely, it is much larger.
